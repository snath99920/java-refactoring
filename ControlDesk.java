...
//    private RunLane runLane;
    ...
    public void assignLane() {
        ...
        while (run.hasNext() && partyQueue.hasMoreElements()) {
            final Lane curLane = (Lane) run.next();

            if (curLane.isPartyAssigned() == false) {
                System.out.println("ok... assigning this party");
                curLane.assignParty((Party) partyQueue.next());
//                runLane.assignParty((Party) partyQueue.next());
            }
        }
    ...
    public void viewScores(final Lane lane) {
    ...
    public void addPartyQueue(final Vector partyNicks) {
        final Vector partyBowlers = new Vector();
        for (int i = 0; i < partyNicks.size(); i++) {
            final Bowler newBowler = registerPatron((String) partyNicks.get(i));
    ...
    public Vector getPartyQueue() {
        final Vector displayPartyQueue = new Vector();
