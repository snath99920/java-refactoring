...
//    private Vector myVector;
    ...
    public EndGameReport(final String partyName, final Party party) {
        ...
        final Iterator iter = party.getMembers().iterator();
        while (iter.hasNext()) {
...
//        Insets buttonMargin = new Insets(4, 4, 4, 4);
    ...
    public void actionPerformed(final ActionEvent actionEvent) {
        if (actionEvent.getSource().equals(printButton)) {
    ...
    public void valueChanged(final ListSelectionEvent select) {
        selectedMember = (String) ((JList) select.getSource()).getSelectedValue();
    }
    ...
    public static void main(final String args[]) {
        final Vector bowlers = new Vector();
        for (int i = 0; i < 4; i++) {
            bowlers.add(new Bowler("aaaaa", "aaaaa", "aaaaa"));
        }
        final Party party = new Party(bowlers);
        String partyName = "wank";
        final EndGameReport finalReport = new EndGameReport(partyName, party);
    }
