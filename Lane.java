...
//    private RunLane runner;
        ...
        while (true) {
            if (partyAssigned && !gameFinished) {	// we have a party on this lane,
                ...
                if (bowlerIterator.hasNext()) {
                    ...
                    if (frameNumber == 9){
                        ...
                        try{
                            ...
                            final String dateString = "" + date.getHours() + ":" + date.getMinutes() + " " + date.getMonth() + "/" + date.getDay() + "/" + (date.getYear() + 1900);
                ...
                if (result == 1) {					// yes, want to play again
                    ...
                    Vector printVector;
                    final EndGameReport egr = new EndGameReport( ((Bowler)party.getMembers().get(0)).getNickName() + "'s Party", party);
                    ...
                    while (scoreIt.hasNext()){
                        ...
                        final ScoreReport scoreReport = new ScoreReport( thisBowler, finalScores[myIndex++], gameNumber );
                        scoreReport.sendEmail(thisBowler.getEmail());
                        final Iterator printIt = printVector.iterator();
     ...
     *
     * @param setEvent 		The pinsetter event that has been received.
     */
    public void receivePinsetterEvent(final PinsetterEvent setEvent) {

        if (setEvent.pinsDownOnThisThrow() >=  0) {			// this is a real throw
            markScore(currentThrower, frameNumber + 1, setEvent.getThrowNumber(), setEvent.pinsDownOnThisThrow());

            // next logic handles the ?: what conditions dont allow them another throw?
            // handle the case of 10th frame first
            if (frameNumber == 9) {
                if (setEvent.totalPinsDown() == 10) {
                    setter.resetPins();
                    if(setEvent.getThrowNumber() == 1) {
                        tenthFrameStrike = true;
                    }
                }

                if (setEvent.totalPinsDown() != 10 && setEvent.getThrowNumber() == 2 && !tenthFrameStrike) {
                    canThrowAgain = false;
                    //publish( lanePublish() );
                }

                if (setEvent.getThrowNumber() == 3) {
                    canThrowAgain = false;
                    //publish( lanePublish() );
                }
            } else { // its not the 10th frame

                if (setEvent.pinsDownOnThisThrow() == 10) {		// threw a strike
                    canThrowAgain = false;
                    //publish( lanePublish() );
                } else if (setEvent.getThrowNumber() == 2) {
                    canThrowAgain = false;
                    //publish( lanePublish() );
                } else if (setEvent.getThrowNumber() == 3)
    ...
    private void resetScores() {
        final Iterator bowlIt = (party.getMembers()).iterator();
    ...
    private void markScore( final Bowler Cur, final int frame, final int ball, final int score ) {
        ...
        int final_score = GetScore.calculateScore(Cur, frame, cumulScores, ball, scores, bowlIndex);
        publish(lanePublish());
    }
    ...
    protected LaneEvent lanePublish(  ) {
        final LaneEvent laneEvent = new LaneEvent(party, bowlIndex, currentThrower, cumulScores, scores, frameNumber+1, curScores, ball, gameIsHalted);
        return laneEvent;
    }
    ...
    public void publish( final LaneEvent event ) {
        if( !subscribers.isEmpty() ) {
            final Iterator eventIterator = subscribers.iterator();
